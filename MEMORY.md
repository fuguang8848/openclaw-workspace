...
### 错误事实
- **行为**: V 在 6/20 09:00 ~ 09:23 多条回复中, 把内部暗语 "修真" (意思是 verify/correct) 当动词替换普通词, 写出了"修真 5 仓""修真 commit""修真 push""修真 修真"这种正常人读不懂的话
- **暴露**: 浮光 09:23 直接问 "你到底在干嘛？""这都是什么？什么修真，修真什么？"
- **根因**: V 沿用了之前 SOP 文档里"修真"作为名词 (修真流程/修真过程) 的用法, 但把它当成动词套进了句子里。读 V 自己写的 SOP 文档能理解, 但读 V 的回复就不能理解。

### 教训 (永久)
- **"修真" 在用户对话中只允许作为名词出现**: 如"修真流程""修真 5 仓""修真 commit 失实"
- **不能作为动词替换普通词**: 不能写"修真 5 仓""修真 commit""修真 push"——要写"核对 5 仓""提交 commit""推送到 origin"
- **自我检测**: 每次回复发出前, grep 自己输出里的 "修真" —— 如果在动词位置, 改掉
- **替代词**: verify → 核对 / 验证, correct → 修正 / 改, check → 查 / 检查, commit → 提交, push → 推送, run → 跑, do → 做
...
### 防范
- 回复前自检: "这段话浮光能直接读懂吗?"
- 修真/核对/验证 类操作, 在对话里只用动词原形 + 宾语 (如"核对 5 仓""核对 PR 状态"), 不要堆砌"修真 修真 修真"
- 如果发现已经在写"修真 修真 修真", 立刻停下来重写
