...
- **暴露**: 浮光 6/20 09:23 直接问 "你到底在干嘛？""这都是什么？什么修真，修真什么？" / 10:18 又问 "你的这个下一步，还有修真？"
...
- 场景 4 (新增): V 在"核对修真"类替换后, 又写出"修真"叠加 (10:18 案例), 仍然是读不通的叠加
...
- **回复前硬性自检**: 每次输出 user-facing 回复前, 必须 grep 自己的草稿
  ...
  ```
  命中即重写, 不发送。
- **常见错误模式**: 输出中出现 "修真 修真" "修真" "修真修真" 等叠加, 立即全部重写为普通中文 (核对 + 提交 + 推送 等)
...
- **触发**: 10:18 浮光问 "你的这个下一步，还有修真？" — 第三次违规 (SOP #42 写完后 1 小时内)
