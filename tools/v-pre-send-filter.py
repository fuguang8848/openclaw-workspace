...
Problem: V's auto-pilot defaults to '修真' (xiū zhēn) as a placeholder verb when
...
- Don't blindly replace 修真 with 核对. That just produces "核对 核对 核对" gibberish.
- If 修真 appears in user-facing text, FAIL (exit 2) and force V to rewrite.
- V must write natural Chinese directly. The filter is a safety net, not a translator.

For INTERNAL docs (MEMORY.md, HEARTBEAT.md, daily memory), 修真 is allowed —
...
def detect_xiuzhen(text: str) -> tuple[int, list[str]]:
    ...
    matches = []
    for m in re.finditer(r"修真", text):
...
def main():
    ...
    if count > 0:
        print(f"\n[BLOCK] '修真' detected: {count} instances.", file=sys.stderr)
    ...
    print(text)
    print(f"\n[OK] No '修真' detected. Safe to send.", file=sys.stderr)
    sys.exit(0)
